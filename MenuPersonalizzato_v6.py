...
myMsgBox_Title2='Procedura: calcola area poligono selezionato'
...
my_destdir = 'C:/TEMP/Lidar_Chiusi_Torrita_Montepulciano/'
    ...
    while a==1:
        ...
        for feature in my_selection:
            try:
                ...
                lyr.setCrs( QgsCoordinateReferenceSystem(feature[my_CRS_system_fld], QgsCoordinateReferenceSystem.EpsgCrsId))
...
def calc_area():
    ...
    Mbox(myMsgBox_Title2, 'Area poligono selezionato: ' +str(myarea), 1)
    


def CopyLidarFromShape():
    ...
    while a==1:
        ...
        if len(my_selection) == 0:
            ...
            print ('Non è stato selezionato nessun oggetto nello shapefile ', my_name_layer)
        for feature in my_selection:
            if path.exists(feature[my_path_fld]):
                shutil.copy2(feature[my_path_fld],my_destdir)
                print ('copiato file n. ', my_file_exist, 'di ', len(my_selection))
                my_file_exist = my_file_exist+1
            else:
                print ('file', my_path_fld, ' non trovato')
                my_file_nonexist = my_file_nonexist+1
        a=0
        Mbox(myMsgBox_Title, 'Procedura terminata. File selezionati: ' +str(len(my_selection))+ 
        '. File copiati: '+str(my_file_exist)+'. File non trovati: '+str(my_file_nonexist) + 
        '. Tempo totale procedura: ' + str(round ((time.time() - start_time),2)) + ' secondi', 1)
...
CopyLidar_action = QAction('Copia Lidar da Shapefile in: '+ my_destdir)
CopyLidar_action.triggered.connect(CopyLidarFromShape)
...
MyMenu.addSeparator()
MyMenu.addAction(CopyLidar_action)
